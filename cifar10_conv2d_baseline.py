...
def train_one_epoch(model, device, dataloader, criterion, optimizer, batch_logger, epoch_idx):
    model.train()
    total_loss = 0.0
    total_correct = 0
    total_samples = 0
...
def main():
    ...
    # ---- NEW: warm-start checkpoint saving options ----
    ap.add_argument(
        "--save_init_path",
        type=str,
        default=None,
    ...
    ap.add_argument("--wandb_project", type=str,
                    default="cifar10_conv2d_baseline",
    ...
    if args.wandb:
        ...
        run_name = f"baseline_conv2d_cifar10_{args.opt}_seed{args.seed}_data{args.data_seed}"

        wandb_config = {
            "method": "baseline_conv2d_cifar10_pytorch",
            "dataset": "CIFAR-10",
            "optimizer": args.opt,
            "seed": int(args.seed),
    ...
    for epoch in range(args.epochs):
        ...
        # ---- NEW: optionally save a warm-start checkpoint for meta-train ----
        if args.save_init_path is not None:
            # Decide whether to save at this epoch.
            # If save_init_epoch is None, save at the last epoch only.
            ...
            if should_save:
                ...
                print(f"[BASELINE] Saved warm-start init to: {ckpt_path}")
    ...
    print(
        f"[BASELINE CONV2D] opt={args.opt} seed={args.seed} data_seed={args.data_seed} "
    ...
    result = {
        "method": f"baseline_conv2d_{args.opt}_pytorch",
