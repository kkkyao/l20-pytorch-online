...
# -*- coding: utf-8 -*-
"""
MNIST-1D Conv1D preprocessing (PyTorch-friendly version).
...
This script:
  ...
  - Optionally exports normalized arrays for Conv1D with shape [N, 1, 40]
    (channels-first layout suitable for torch.nn.Conv1d).
"""
...
def sha256_of_array(a: np.ndarray, max_bytes: int = 8_000_000) -> str:
    ...
    For very large arrays, only the first `max_bytes` of the raw buffer
    are used, which is usually sufficient as a fingerprint.
    """
    h = hashlib.sha256()
    b = a.tobytes()
...
def compute_norm_stats(x_train_subset: np.ndarray, eps: float = 1e-6):
    """
    Compute per-position mean and std on the TRAIN subset.

    x_train_subset: [N_train, 40]
    Returns:
      mean: [40]
...
def apply_normalization(x: np.ndarray, mean: np.ndarray, std: np.ndarray):
    """
    Apply per-position standardization.
...
def to_conv1d_torch(x: np.ndarray) -> np.ndarray:
    """
    Convert [N, 40] to [N, 1, 40] for torch.nn.Conv1d (channels-first).
    """
    return x[:, None, :]


def save_json(obj: dict, path: Path):
    """Save a Python dict as a pretty-printed JSON file."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Preprocess MNIST-1D for Conv1D (PyTorch-friendly)."
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
        help="random seed for stratified split and data loader",
    )
    parser.add_argument(
        "--val_frac",
        type=float,
        default=0.1,
        help="validation fraction taken from the original training set",
    )
    parser.add_argument(
        "--artifacts_dir",
        type=str,
    ...
    parser.add_argument(
        "--export_npz",
        action="store_true",
        help="also export normalized arrays for Conv1D",
    )
    parser.add_argument(
        "--npz_name",
        type=str,
        default=None,
        help="custom filename for exported npz (optional)",
    )
    ...
    # 1) Load data and normalize shapes to [N, 40]
    (x_tr, y_tr), (x_te, y_te) = load_mnist1d(length=40, seed=seed)

    def to_N40(x: np.ndarray) -> np.ndarray:
        """Normalize shape to [N, 40], supporting [N,40], [N,40,1], [N,1,40]."""
        x = np.asarray(x)
        ...
        raise AssertionError(
            f"Unexpected shape for x: {x.shape}, expected [N,40] or [N,40,1]/[N,1,40]"
        )
    ...
    # 2) Stratified train/val split (from the original training set only)
    ...
    # 7) Optional: export normalized Conv1D-ready arrays
    if args.export_npz:
        ...
        x_train_c1d = to_conv1d_torch(x_train_norm)   # [N_train, 1, 40]
        x_val_c1d = to_conv1d_torch(x_val_norm)       # [N_val, 1, 40]
        x_test_c1d = to_conv1d_torch(x_test_norm)     # [N_test, 1, 40]

        npz_name = args.npz_name or f"mnist1d_seed{seed}_conv1d_torch_normed.npz"
        ...
        print(f"[OK] Exported normalized Conv1D arrays to: {npz_path.resolve()}")
        print(
            f"    x_train: {x_train_c1d.shape}, "
            f"x_val: {x_val_c1d.shape}, x_test: {x_test_c1d.shape}"
        )
