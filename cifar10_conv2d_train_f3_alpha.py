...
This is the CIFAR-10 Conv2D counterpart of an F3 α-mix BF-style learned step-size:
...
Key design:
  - For each train batch:
      ...
      3) Build F3 α-mix feature phi_t from g_t and m_t
      4) Sample a val batch, compute val_loss and grad_val
      5) Compute dot = <grad_val, g_t> (still along raw gradient direction)
      ...
      8) With the updated LearnedL, recompute eta and update w
         using an α-mix direction d_t (no momentum optimizer object)
  ...
  - Data preprocessing, normalization, and log file naming follow the
    same conventions as the F1/F3 versions, with CIFAR-specific preprocess
...
class BatchLossLogger:
    """
    Lightweight BatchLossLogger aligned with the CIFAR-10 Conv2D BF variants.
...
def main():
    ...
    # Learner / optimizer hyperparameters (aligned with F1/F3, plus EMA beta and alpha_mix)
    ...
    # Clamp alpha_mix into [0, 1] for safety.
    alpha = float(np.clip(args.alpha_mix, 0.0, 1.0))
    print(f"[INFO] Using alpha_mix = {alpha:.3f}")

    # Run directory: encode CIFAR-10 + Conv2D + F3 α-mix
    run_name = (
    ...
    curve_logger = BatchLossLogger(
        run_dir,
        meta={
            "method": "learned_l_f3_alphamix_cifar10_conv2d",
            "seed": args.seed,
            "opt": "learnedL",
            "lr": args.theta_lr,
        },
    ...
    for epoch in range(args.epochs):
        ...
        for xb, yb in train_loader:
            ...
            # --- 3) Compute F3 α-mix features ---
            # Scaled gradient and momentum
            g_scaled = [(1.0 - alpha) * g.detach() for g in grads]
            m_scaled = [alpha * m for m in m_buffers]

            # Norms
            g_norm_scaled_sq = sum((gs ** 2).sum() for gs in g_scaled)
            g_norm_scaled = torch.sqrt(g_norm_scaled_sq + args.eps)

            m_norm_scaled_sq = sum((ms ** 2).sum() for ms in m_scaled)
            m_norm_scaled = torch.sqrt(m_norm_scaled_sq + args.eps)
            ...
            phi_g = torch.log(g_norm_scaled + args.eps)
            phi_m = torch.log(m_norm_scaled + args.eps)
            phi_diff = diff_norm
            phi = torch.stack([phi_g, phi_m, phi_diff], dim=0).view(1, 3)  # [1, 3]
            ...
            # --- 6) Clip direction d_t and update w using the *updated* theta ---
            # α-mix direction: d_t = (1 - alpha) g_t + alpha m_t
            d_t = [
                (1.0 - alpha) * g.detach() + alpha * m
                for g, m in zip(grads, m_buffers)
            ]

            # global norm for d_t (for clipping)
            d_norm_for_clip = torch.sqrt(
                sum((d ** 2).sum() for d in d_t) + 1e-12
            )
            if args.clip_grad is not None and args.clip_grad > 0.0:
                if d_norm_for_clip.item() > args.clip_grad:
                    clip_coef = args.clip_grad / float(d_norm_for_clip.item())
                else:
                    clip_coef = 1.0
            else:
                clip_coef = 1.0
            ...
            with torch.no_grad():
                ...
                for p, d in zip(params, d_t):
                    ...
                    p.data -= eta_scalar_now.to(p.device).to(p.dtype) * d_update
    ...
    print(
        f"[RESULT-CIFAR10-Conv2D-F3Alpha-PT] TestAcc={final_test_acc:.4f} "
