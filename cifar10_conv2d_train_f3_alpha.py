...
# -*- coding: utf-8 -*-
"""
BF-style Step-3 (B+F3 α-mix) for CIFAR-10 + Conv2D -- ONLINE TRAIN (PyTorch)
-----------------------------------------------------------------------------

F3 α-mix:
  - EMA of gradients m_t = beta*m_{t-1} + (1-beta)*g_t
  - Feature:
        phi_t = [
            log ||(1-alpha) g_t||,
            log ||alpha m_t||,
  ...
  - Update direction: d_t = (1-alpha) g_t + alpha m_t

Aligned stability knobs with F1/F2/F3:
  ...
  - Per-epoch eta/L/phi/dot statistics logged to eta_stats_f3alpha.csv
"""
...
class Conv2DBaseline(nn.Module):
    """
    Match your F1 Conv2D backbone exactly (32/64/128).
    """
    def __init__(self):
        super().__init__()
...
class LearnedL(nn.Module):
    """
    phi = [log||(1-a)g||, log||a m||, ||m-g||] -> L_theta in [Lmin, Lmax]
    """
...
def main():
    ...
    ap.add_argument("--alpha_mix", type=float, default=1.0, help="alpha in [0,1]")

    # aligned stability / regularization knobs (like F1)
    ...
    ap.add_argument("--wandb_group", type=str, default="cifar10_conv2d_f3alpha_bf1pt")
    ...
    alpha = float(np.clip(args.alpha_mix, 0.0, 1.0))
    print(f"[INFO] Using alpha_mix = {alpha:.3f}")

    run_name = (
        f"cifar10_conv2d_f3alpha_data{args.data_seed}_seed{args.seed}_alpha{alpha:.3f}_"
    ...
    if args.wandb:
        if wandb is None:
            raise RuntimeError("wandb not installed but --wandb was passed.")
        wandb_config = {
            "stage": "online-train",
            "dataset": "CIFAR-10",
            "backbone": "Conv2D",
    ...
    curve_logger = BatchLossLogger(
        run_dir,
        meta={"method": "learned_l_f3alpha_cifar10_conv2d_online_pt", "seed": args.seed, "opt": "learnedL", "lr": args.theta_lr},
    )
    mech_path = run_dir / "mechanism_f3alpha.csv"
    eta_stats_path = run_dir / "eta_stats_f3alpha.csv"
    ...
    with open(mech_path, "w") as f:
        f.write("iter,epoch,eta_t,L_theta,phi_g,phi_m,phi_diff,alpha_mix\n")
    ...
    for epoch in range(args.epochs):
        ...
        for xb, yb in train_loader:
            ...
            # 3) feature phi: log||(1-a)g||, log||a m||, ||m-g||
            # scaled norms (avoid degenerate alpha=0/1 by eps inside sqrt and log)
            g_scaled_norm_sq = sum((((1.0 - alpha) * g.detach()) ** 2).sum() for g in grads)
            g_scaled_norm = torch.sqrt(g_scaled_norm_sq + args.eps)

            m_scaled_norm_sq = sum(((alpha * m) ** 2).sum() for m in m_buffers)
            m_scaled_norm = torch.sqrt(m_scaled_norm_sq + args.eps)

            diff_norm_sq = sum(((m - g.detach()) ** 2).sum() for m, g in zip(m_buffers, grads))
            diff_norm = torch.sqrt(diff_norm_sq + args.eps)

            phi_g = torch.log(g_scaled_norm + args.eps)
            phi_m = torch.log(m_scaled_norm + args.eps)
            phi_diff = diff_norm
            ...
            # 6) update w with alpha-mix direction, but apply eta limiter + wd
            d_t = [(1.0 - alpha) * g.detach() + alpha * m for g, m in zip(grads, m_buffers)]

            d_norm_for_clip = torch.sqrt(sum((d ** 2).sum() for d in d_t) + 1e-12)
            if args.clip_grad is not None and args.clip_grad > 0.0:
                clip_coef = args.clip_grad / float(d_norm_for_clip.item()) if d_norm_for_clip.item() > args.clip_grad else 1.0
            else:
                clip_coef = 1.0
            ...
            with torch.no_grad():
                ...
                for p, d in zip(params, d_t):
                    d_update = d * clip_coef
                    if args.wd is not None and args.wd > 0.0:
                        d_update = d_update + args.wd * p.data
                    p.data -= eta_limited.to(p.device).to(p.dtype) * d_update
            ...
            pg_hist.append(float(phi_g.item()))
            pm_hist.append(float(phi_m.item()))
            pd_hist.append(float(phi_diff.item()))
        ...
        pg_arr = np.asarray(pg_hist, dtype=np.float64) if pg_hist else np.asarray([np.nan])
        pm_arr = np.asarray(pm_hist, dtype=np.float64) if pm_hist else np.asarray([np.nan])
        pd_arr = np.asarray(pd_hist, dtype=np.float64) if pd_hist else np.asarray([np.nan])
        ...
        phi_diff_mean = float(np.nanmean(pd_arr))
        ...
        with open(eta_stats_path, "a") as f:
            f.write(
                ...
                f"{L_mean:.8g},{phi_g_mean:.8g},{phi_m_mean:.8g},{phi_diff_mean:.8g},{dot_mean:.8g}\n"
            )

        print(
            f"[CIFAR10-Conv2D-F3Alpha-PT EPOCH {epoch}] "
            ...
            wandb.log(
                {
                    ...
                    "phi/phi_diff_mean": phi_diff_mean,
                }
            )
    ...
    print(
        f"[RESULT-CIFAR10-Conv2D-F3Alpha-PT] "
