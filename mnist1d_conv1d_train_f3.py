...
# -*- coding: utf-8 -*-
"""
PyTorch implementation of Step-3 (B+F3) for MNIST-1D with a Conv1D backbone.
...
Key design:
  - Closely follows the F1 online meta-learning logic but with:
      ...
      * Features: phi_t = [log ||g_t||, log ||m_t||, ||m_t - g_t||]
  ...
  - For each train batch:
      ...
      3) Build phi_t from norms of g_t, m_t, and (m_t - g_t)
  ...
  - Data preprocessing, normalization, and log file naming are aligned
    with the F1/F2 PyTorch implementations.
"""
...
class LearnedL(nn.Module):
    """
    MLP mapping phi = [log ||g||, log ||m||, ||m - g||]
...
def main():
    ...
    # Learner hyperparameters (aligned with the F1/F2 implementations)
    ...
    ap.add_argument(
        "--wandb_group",
        type=str,
        default="mnist1d_conv1d_f3_bf_online",
    ...
    # Run directory: keep naming aligned with Conv1D BF F* scripts
    run_name = (
        f"mnist1d_conv1d_f3_data{args.data_seed}_seed{args.seed}_"
    ...
    if args.wandb:
        ...
        wandb_config = {
            ...
            "dataset": "MNIST-1D",
            "backbone": "Conv1D",
            "method": "learned_l_f3_mnist1d_conv1d_online_pt",
            "length": args.length,
            "seed": args.seed,
    ...
    curve_logger = BatchLossLogger(
        run_dir,
        meta={
            "method": "learned_l_f3_mnist1d_conv1d",
            "seed": args.seed,
            "opt": "learnedL",
            "lr": args.theta_lr,
        },
    ...
    for epoch in range(args.epochs):
        ...
        for xb, yb in train_loader:
            ...
            # Compute norms for g_t, m_t, and (m_t - g_t)
            ...
            # phi_t = [log ||g_t||, log ||m_t||, ||m_t - g_t||]
            phi = torch.stack(
                [log_g_norm, log_m_norm, mg_diff], dim=0
            ).view(1, 3)  # [1, 3]
            ...
            # Warmup logic aligned with the F1/F2 implementations
    ...
    print(
        f"[RESULT-MNIST1D-Conv1D-F3-PT] TestAcc={final_test_acc:.4f} "
