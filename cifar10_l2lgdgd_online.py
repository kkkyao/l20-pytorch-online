...
def clip_delta_vec(delta_vec: torch.Tensor, clip_update: Optional[float], eps: float = 1e-12):
    """
    Differentiable global-norm clipping for Δθ.
...
class TinyCNN(nn.Module):
    """
    Small CNN for CIFAR-10 to keep coordinate-wise LSTM states feasible.
    Roughly ~150k-200k parameters depending on widths.
    """
...
class LearnedOptimizer(nn.Module):
    ...
    Preprocess:
      if |g| >= exp(-p): (log(|g|)/p, sign(g))
      else:             (-1, exp(p)*g)

    This is first-order: grad input is detached (no 2nd derivatives).
    """
    ...
    def _preprocess(self, g: torch.Tensor) -> torch.Tensor:
        gd = g.detach()  # first-order (drop dashed edges)
...
def main():
    ...
    # stability knobs
    ap.add_argument("--clip_update", type=float, default=0.1)     # Δθ clip
    ap.add_argument("--reg_update", type=float, default=1e-6)     # L2 reg on raw update
    ap.add_argument("--clip_phi_grad", type=float, default=1.0)   # phi grad clip

    # state reset controls (optional)
    ...
    val_eval_loader = DataLoader(val_set, batch_size=512, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    test_eval_loader = DataLoader(test_set, batch_size=512, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    ...
    print(f"[INFO] optimizee params = {n_params}")
            ...
            wandb.log(
                {
                    ...
                    "meta/clip_update": float(args.clip_update),
                    "meta/out_mul": float(args.out_mul),
                }
            )
