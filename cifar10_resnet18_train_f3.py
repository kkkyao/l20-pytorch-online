...
# -*- coding: utf-8 -*-
"""
PyTorch BF-style (B+F3) online meta-learning on CIFAR-10 with a ResNet-18 backbone.
-----------------------------------------------------------------------------------

This script is the CIFAR-10 ResNet-18 counterpart of the Conv2D / MNIST-1D F3 BF-style code.
...
Key design:
  - For each train batch:
      ...
      3) Build F3 feature phi_t from g_t and m_t
      4) Sample a val batch, compute val_loss and grad_val
      5) Compute dot = <grad_val, m_t> (direction = momentum, as in BF2)
      ...
      8) With the updated LearnedL, recompute eta and update w
         using the momentum direction m_t with global-norm clipping
  ...
  - Data preprocessing, normalization, and log file naming are aligned
    with the Conv2D / ResNet F1/F2 implementations, using artifacts under
...
def main():
    ...
    # Learner / optimizer hyperparameters (aligned with F2, plus momentum beta)
    ...
    ap.add_argument(
        "--wandb_group",
        type=str,
        default="cifar10_resnet18_f3_bf3pt",
    ...
    # Run directory: encode CIFAR-10 + ResNet18 + F3
    run_name = (
        f"cifar10_resnet18_f3_data{args.data_seed}_seed{args.seed}_"
    ...
    if args.wandb:
        ...
        wandb_config = {
            ...
            "dataset": "CIFAR-10",
            "backbone": "ResNet18",
            "method": "learned_l_f3_cifar10_resnet18_online_pt",
            "seed": args.seed,
    ...
    curve_logger = BatchLossLogger(
        run_dir,
        meta={
            "method": "learned_l_f3_cifar10_resnet18",
            "seed": args.seed,
            "opt": "learnedL",
            "lr": args.theta_lr,
        },
    ...
    # ---------------- training loop (online meta-learning, F3) ----------------
    for epoch in range(args.epochs):
        ...
        for xb, yb in train_loader:
            ...
            # dot = sum <grad_val, m_t> (momentum direction, as in BF2)
            dot = torch.zeros([], device=device, dtype=torch.float32)
            for gv, m in zip(grad_val, m_buffers):
                dot = dot + (gv.float() * m.float()).sum()

            # --- 5) Online meta-update on theta using F3 feature ---
            phi_in = phi.to(device)
            L_theta = learner(phi_in)  # [1, 1]
            eta = args.c_base / (L_theta + args.eps)

            if global_step < args.warmup_steps:
                # Warmup logic aligned with F1/F2 implementations
                warmup_max = min(
                    args.eta_max,
            ...
            m_norm_for_clip = torch.sqrt(
                sum((m ** 2).sum() for m in m_buffers) + 1e-12
            )
        ...
        print(
            f"[CIFAR10-ResNet18-F3-PT EPOCH {epoch}] "
    ...
    print(
        f"[RESULT-CIFAR10-ResNet18-F3-PT] TestAcc={final_test_acc:.4f} "
