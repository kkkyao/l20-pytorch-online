...
# -*- coding: utf-8 -*-
"""
Step-3 (B+F3, alpha-mix) on MNIST-1D with an MLP backbone
(PyTorch, single-task online meta-learning)
-----------------------------------------------------------------

Backbone:
  - MLP on 1D input of length `length` (default 40).

Learned step-size network L_theta with F3 alpha-mix features:
  phi_t = [
    log || (1 - alpha) g_t ||,
    log || alpha m_t ||,
    || m_t - g_t ||
...
Training logic (per train batch):
  ...
  3) Build F3 alpha-mix features phi_t
  ...
  8) With the updated LearnedL, recompute eta and update w using the
     momentum direction (no optimizer with internal momentum)
...
class BatchLossLogger:
    """
    Lightweight BatchLossLogger for per-batch training loss.
...
# ------------------------- Learned L_theta (B+F3 alpha-mix) -------------------------
...
def main():
    ...
    # Learner hyperparameters (aligned with other F1/F3 scripts)
    ...
    # Momentum + alpha-mix hyperparameters
    ...
    ap.add_argument(
        "--wandb_group",
        type=str,
        default="mnist1d_mlp_f3alpha_bf_online",
    ...
    print(f"[INFO] alpha_mix = {args.alpha_mix:.3f}")
    set_seed(args.seed)

    # Run directory: encode that this is MNIST1D + MLP + F3 alpha-mix
    run_name = (
        f"mnist1d_mlp_f3alpha_data{args.data_seed}_seed{args.seed}_"
    ...
    if args.wandb:
        ...
        wandb_config = {
            ...
            "dataset": "MNIST-1D",
            "backbone": "MLP",
            "method": "learned_l_f3_alpha_mlp_online_pt",
            "length": args.length,
            "seed": args.seed,
    ...
    curve_logger = BatchLossLogger(
        run_dir,
        meta={
            "method": "learned_l_f3_alpha_mnist1d_mlp",
            "seed": args.seed,
            "opt": "learnedL",
            "lr": args.theta_lr,
        },
    ...
    result_path = run_dir / "result_f3_alpha_mlp.json"

    with open(mech_path, "w") as f:
        f.write(
            "iter,epoch,eta_t,L_theta,"
            "log_g_norm_scaled,log_m_norm_scaled,mg_diff,alpha_mix\n"
        )
    ...
    for epoch in range(args.epochs):
        ...
        for xb, yb in train_loader:
            ...
            # --- 3) Build alpha-mix features phi_t ---
            alpha = args.alpha_mix

            # scaled gradients and momentum for feature norms
            g_scaled = [(1.0 - alpha) * g.detach() for g in grads]
            m_scaled = [alpha * m.detach() for m in momentum]

            g_norm_sq = sum((gs ** 2).sum() for gs in g_scaled)
            g_norm_scaled = torch.sqrt(g_norm_sq + args.eps)

            m_norm_sq = sum((ms ** 2).sum() for ms in m_scaled)
            m_norm_scaled = torch.sqrt(m_norm_sq + args.eps)

            mg_diff_sq = sum(((m.detach() - g.detach()) ** 2).sum()
                             for m, g in zip(momentum, grads))
            mg_diff = torch.sqrt(mg_diff_sq + args.eps)

            log_g_norm_scaled = torch.log(g_norm_scaled + args.eps)
            log_m_norm_scaled = torch.log(m_norm_scaled + args.eps)

            # phi_t = [log ||(1 - alpha) g_t||, log ||alpha m_t||, ||m_t - g_t||]
            phi = torch.stack(
                [log_g_norm_scaled, log_m_norm_scaled, mg_diff], dim=0
            ).view(1, 3)  # [1, 3]
            ...
            # --- 6) Clip momentum and update w using the *updated* theta ---
            ...
            with torch.no_grad():
                ...
                with open(mech_path, "a") as f:
                    f.write(
                        ...
                        f"{float(log_g_norm_scaled.item()):.6g},"
                        f"{float(log_m_norm_scaled.item()):.6g},"
                        f"{float(mg_diff.item()):.6g},"
                        f"{args.alpha_mix:.6g}\n"
                    )
        ...
        print(
            f"[MNIST1D-MLP-F3-ALPHA-PT EPOCH {epoch}] "
    ...
    print(
        f"[RESULT-MNIST1D-MLP-F3-ALPHA-PT] TestAcc={final_test_acc:.4f} "
