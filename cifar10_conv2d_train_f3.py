...
# -*- coding: utf-8 -*-
"""
BF-style Step-3 (B+F3) for CIFAR-10 + Conv2D -- ONLINE TRAIN (PyTorch)
-----------------------------------------------------------------------

F3 feature:
  phi_t = [log ||g_t||, log ||m_t||, ||m_t - g_t||]
...
Online meta-learning loop (single task):
  For each train batch:
    ...
    4) Compute dot = <grad_val, g_t> (averaged over K)
    5) meta_loss = val_loss - eta * dot + small regularizer
    6) Update theta (LearnedL params)
    7) Recompute eta with updated theta and update w with SGD-like step
       with global-norm clipping (+ optional weight decay)

Stability additions (aligned with your F1/F2):
  ...
  - Per-epoch eta/L/phi/dot statistics logged to eta_stats_f3.csv
"""
...
class LearnedL(nn.Module):
    """
    Learned scalar L_theta for F3 feature:
      phi = [log||g||, log||m||, ||m-g||]
...
def main():
    ...
    # Step-size learner hyperparameters (aligned with F1/F2, plus EMA beta)
    ...
    # --- Stability / regularization knobs (match F1/F2) ---
    ...
    ap.add_argument("--wandb_group", type=str, default="cifar10_conv2d_f3_bf1pt", help="WandB group name")
    ...
    # ------------------------------------------------------------------
    run_name = (
        f"cifar10_conv2d_f3_data{args.data_seed}_seed{args.seed}_"
    ...
    if args.wandb:
        ...
        wandb_config = {
            ...
            "dataset": "CIFAR-10",
            "backbone": "Conv2D",
            "method": "learned_l_f3_cifar10_conv2d_online_pt",
            "seed": args.seed,
    ...
    train_eval_loader = make_eval_loader(train_dataset, batch_size=512, num_workers=0, pin_memory=False)
    ...
    # ------------------------------------------------------------------
    # Training loop (online meta-learning BF3-style)
    # ------------------------------------------------------------------
    for epoch in range(args.epochs):
        ...
        print(
            f"[CIFAR10-Conv2D-F3-PT EPOCH {epoch}] "
