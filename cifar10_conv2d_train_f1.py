...
# -*- coding: utf-8 -*-
"""
PyTorch BF-style (B+F1) online meta-learning on CIFAR-10 with a Conv2D backbone.
-------------------------------------------------------------------------------

This is the CIFAR-10 Conv2D counterpart of mnist1d_conv1d_train_f1.py.
...
Key design:
  ...
  - Data preprocessing, normalization, and log file naming follow the
    same conventions as the MNIST-1D version, but with CIFAR-specific
    preprocess artifacts under preprocess_dir (e.g., artifacts/cifar10_conv2d_preprocess).
"""
...
class BatchLossLogger:
    """
    Lightweight BatchLossLogger aligned with the MNIST-1D F1 version.
...
def load_artifacts(preprocess_dir: Path, seed: int):
    ...
    Load preprocessing artifacts for CIFAR-10 Conv2D:
      ...
      - norm.json:  {mean, std}  (typically per-channel mean/std)
...
def build_model(num_channels: int = 3) -> nn.Module:
    ...
    A reasonable architecture aligned with typical CIFAR-10 ConvNets:
    ...
    If your existing baseline uses a slightly different Conv2D backbone,
    you can replace this definition with that one to keep architectures
    exactly aligned.
    """
...
def main():
    ...
    # Learner hyperparameters (aligned with MNIST-1D F1 implementation)
    ...
    # Run directory: encode CIFAR-10 + Conv2D + F1
    run_name = (
    ...
    curve_logger = BatchLossLogger(
        run_dir,
        meta={
            "method": "learned_l_f1_cifar10_conv2d",
            "seed": args.seed,
            "opt": "learnedL",
            "lr": args.theta_lr,
        },
    ...
    for epoch in range(args.epochs):
        ...
        for xb, yb in train_loader:
            ...
            if global_step < args.warmup_steps:
                # Warmup logic aligned with the MNIST-1D F1 implementation
                warmup_max = min(
                    args.eta_max,
    ...
    print(
        f"[RESULT-CIFAR10-Conv2D-F1-PT] TestAcc={final_test_acc:.4f} "
