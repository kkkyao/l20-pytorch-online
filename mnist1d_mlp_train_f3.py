...
# -*- coding: utf-8 -*-
"""
Step-3 (B+F3) on MNIST-1D with an MLP backbone (PyTorch, single-task online meta-learning)
-----------------------------------------------------------------------------------------

Features:
  - Backbone: MLP on 1D input of length `length` (default 40)
  - Learned step-size network L_theta with F3 features:
        phi_t = [log ||g_t||, log ||m_t||, ||m_t - g_t||]
  - Step-size:
        eta_t = c_base / (L_theta(phi_t) + eps)
  - Single-task online meta-learning with momentum update:
        m_t = beta * m_{t-1} + (1 - beta) * g_t

Training logic (per train batch):
  1) Compute train loss and gradient g_t on the train batch
  2) Update momentum m_t
  3) Build F3 features phi_t from norms of g_t, m_t, and (m_t - g_t)
  ...
  8) With the updated LearnedL, recompute eta and update w using the
     momentum direction (no standard optimizer with momentum)
...
class BatchLossLogger:
    """
    Lightweight BatchLossLogger (same structure as Conv1D F3 version).
...
def main():
    ...
    # Learner hyperparameters (aligned with Conv1D F3 implementation)
    ...
    result_path = run_dir / "result_f3_mlp.json"
    ...
    for epoch in range(args.epochs):
        ...
        for xb, yb in train_loader:
            ...
            # Warmup logic aligned with Conv1D F3 implementation
    ...
    result = {
        ...
        "backbone": "MLP",
        "method": "learned_l_f3_mlp_online_pt",
