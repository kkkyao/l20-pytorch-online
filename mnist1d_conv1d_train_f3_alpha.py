...
# -*- coding: utf-8 -*-
"""
PyTorch implementation of Step-3 (B+F3, alpha-mix) for MNIST-1D with a Conv1D backbone.
...
B+F3 alpha-mix:
  ...
  - Single-step (T=1) online meta-learning on a *single task*,
    using momentum m_t as the update direction.

Design:
  - Closely follows the PyTorch F1/F2/F3 online meta-learning scripts:
      * Uses MNIST-1D with Conv1D backbone and fixed preprocess artifacts
        (split.json + norm.json) for train/val/test.
      * For each train batch:
          1) Compute train loss and gradient g_t
          ...
          8) With the updated LearnedL, recompute eta and update w
             using the momentum direction (no momentum optimizer)
...
def build_model(input_len: int = 40) -> nn.Module:
    """
    Conv1D backbone aligned with the other MNIST-1D Conv1D scripts:
...
def main():
    ...
    # Learner hyperparameters (aligned with F1/F2/F3 implementations)
    ...
    # Momentum and alpha-mix hyperparameters
    ...
    for epoch in range(args.epochs):
        ...
        for xb, yb in train_loader:
            ...
            # Same warmup logic as F1/F2/F3 scripts
